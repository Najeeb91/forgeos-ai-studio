...
try {
  ...
  await pool.query("INSERT INTO approval_requests(id,project_id,run_id,step_id,action_type,target,reason,risk,status) VALUES($1,$2,$3,$4,'deploy_production','e2e','E2E lifecycle release gate','critical','pending')",[deployApprovalId, projectId, runId, deployStepId]);
  ...
  await transitionRun(pool, runId, "deploying", { eventStage: "deploy", message: "E2E deployment adapter started." });

  const deploymentId = randomUUID();
  await pool.query("INSERT INTO deployments(id,project_id,run_id,env,status,commit_sha,url,adapter) VALUES($1,$2,$3,'production','ready',$4,$5,'e2e-simulated')",[deploymentId, projectId, runId, result.snapshotId, "https://e2e.invalid/verified-run"]);
  await transitionRun(pool, runId, "deployed", { eventStage: "deploy", message: "E2E deployment lifecycle completed." });
  ...
  console.log(JSON.stringify({ok:true,simulatedDeploymentOnly:true,realBuild:true,lifecycle:["project","approval","generation","real-build","test","review","deployment-approval","deploying","deployed"],runId,projectId,snapshotId:result.snapshotId}));
} finally {
  await pool.end();
}
