...
const vercelProject = (
  process.env.FORGEOS_E2E_VERCEL_PROJECT || "forgeos-universal-ai-factory"
).toLowerCase();
...
try {
  ...
  const prompt =
    "Build a small verified ForgeOS generated React application for the end-to-end lifecycle test.";
  ...
  const preview = await deployVercel({
    token: vercelToken,
  ...
  for (let i = 0; i < 18 && !["READY", "ERROR", "CANCELED"].includes(previewStatus.state); i++) {
    ...
    previewStatus = await getVercelDeployment({
      token: vercelToken,
  ...
  await pool.query(
    "INSERT INTO approval_requests(id,project_id,run_id,step_id,action_type,target,reason,risk,status) VALUES($1,$2,$3,$4,'deploy_production','production','E2E lifecycle release gate','critical','pending')",
  ...
  assertEqual(
    deployApproved.state,
    "review",
    "production approval must preserve review state until deployment starts",
  );
  await transitionRun(pool, runId, "deploying", {
    eventStage: "deploy",
    message: "E2E real deployment adapter started.",
  });

  const production = await deployVercel({
    token: vercelToken,
    projectName: vercelProject,
    files: result.sourceFiles,
  ...
  for (let i = 0; i < 24 && !["READY", "ERROR", "CANCELED"].includes(productionStatus.state); i++) {
    await new Promise((resolve) => setTimeout(resolve, 5000));
    productionStatus = await getVercelDeployment({
      token: vercelToken,
  ...
  await pool.query(
    "INSERT INTO deployments(id,project_id,run_id,env,status,commit_sha,url,adapter,simulated) VALUES($1,$2,$3,'production','ready',$4,$5,'vercel',false)",
    [deploymentId, projectId, runId, result.snapshotId, productionStatus.url],
  );
  await transitionRun(pool, runId, "deployed", {
    eventStage: "deploy",
    message: "Real Vercel deployment lifecycle completed.",
  });

  const cancelRunId = randomUUID();
  await pool.query(
    "INSERT INTO ai_runs(id,project_id,prompt,status) VALUES($1,$2,$3,'executing')",
    [cancelRunId, projectId, "Cancellation-path verification"],
  );
  await transitionRun(pool, cancelRunId, "cancelled", {
    eventStage: "cancelled",
    message: "E2E cancellation requested.",
  });
  const cancelled = (await pool.query("SELECT status FROM ai_runs WHERE id=$1", [cancelRunId]))
    .rows[0];
  ...
  console.log(
    JSON.stringify({
      ok: true,
      simulatedDeploymentOnly: false,
      realBuild: true,
      realPreview: true,
      realProductionDeploy: true,
      cancellation: true,
      lifecycle: [
        "project",
        "approval",
      ...
      productionUrl: productionStatus.url,
    }),
  );
} finally {
  await pool.end();
