...
export async function readProject(pool, slug) {
  ...
  const contextEntries = (await pool.query("select id,kind,title,content,source,occurred_at from project_context_entries where project_id=$1 order by occurred_at desc", [p.id])).rows;
  const memory = (await pool.query("select id,kind,title,content,author_type,author_id,source,occurred_at,supersedes_id from project_memory_entries where project_id=$1 and status='active' order by occurred_at desc limit 200", [p.id])).rows;
  const conversation = (await pool.query("select id,title from conversations where project_id=$1 order by updated_at desc limit 1", [p.id])).rows[0];
  const messages = conversation ? (await pool.query("select id,role,content,created_at from conversation_messages where conversation_id=$1 order by created_at", [conversation.id])).rows : [];
  const approvals = (await pool.query("select id,run_id,step_id,action_type,target,reason,risk,status,actor_id,decision_reason,created_at,decided_at from approval_requests where project_id=$1 order by created_at desc limit 100", [p.id])).rows;
  const providerAttempts = (await pool.query("select id,run_id,kind,provider,capability,status,priority,job_id,simulated,error,started_at,completed_at from provider_attempts where project_id=$1 order by started_at desc limit 100", [p.id])).rows;
  const deploymentHistory = (await pool.query("select id,env,status,commit_sha,url,adapter,created_at from deployments where project_id=$1 order by created_at desc limit 100", [p.id])).rows;
  const brainRow = (await pool.query("select vision,requirements,decisions,architecture,schema,integrations,version from project_brain_versions where project_id=$1 order by version desc limit 1", [p.id])).rows[0];
  const runs = (await pool.query("select id,prompt,provider,model,status,started_at,completed_at,tokens_in,tokens_out from ai_runs where project_id=$1 order by started_at desc limit 20", [p.id])).rows;
  const runIds = runs.map((r) => r.id);
  const steps = runIds.length ? (await pool.query("select id,run_id,title,detail,stage,risk,status,order_idx from ai_run_steps where run_id=any($1::uuid[]) order by order_idx", [runIds])).rows : [];
  const events = runIds.length ? (await pool.query("select id,run_id,level,stage,message,created_at from ai_events where run_id=any($1::uuid[]) order by created_at", [runIds])).rows : [];
  const audit = (await pool.query("select id,actor,actor_name,action,target,risk,approved,diff_summary,stage,created_at from audit_events where project_id=$1 order by created_at desc limit 200", [p.id])).rows;
  const latest = (await pool.query("select ss.id as snapshot_id,ss.created_at,sf.path,sf.language,sf.content from source_snapshots ss left join source_files sf on sf.snapshot_id=ss.id where ss.project_id=$1 order by ss.created_at desc,sf.path", [p.id])).rows;
  const latestSnapshotId = latest[0]?.snapshot_id;
  const files = latest.filter((r) => r.snapshot_id === latestSnapshotId && r.path).map((r) => ({path: r.path, language: r.language || "text", content: r.content || ""}));
  return { project: {
    id: p.id, slug: p.slug, name: p.name, tagline: p.tagline, description: p.description,
    status: p.status, health: p.health, owner: p.owner, stack: p.stack || [], benchmark: Boolean(p.benchmark), createdAt: p.created_at, updatedAt: p.updated_at,
    contextHistory: contextEntries.map((e) => ({id:e.id, kind:e.kind, title:e.title, content:e.content, source:e.source, occurredAt:e.occurred_at})),
    memory: memory.map((e) => ({id:e.id, kind:e.kind, title:e.title, content:e.content, authorType:e.author_type, authorId:e.author_id, source:e.source, occurredAt:e.occurred_at, supersedesId:e.supersedes_id})),
    approvals: approvals.map((a) => ({id:a.id, runId:a.run_id, stepId:a.step_id, actionType:a.action_type, target:a.target, reason:a.reason, risk:a.risk, status:a.status, actorId:a.actor_id, decisionReason:a.decision_reason, createdAt:a.created_at, decidedAt:a.decided_at})),
    providerAttempts: providerAttempts.map((a) => ({id:a.id, runId:a.run_id, kind:a.kind, provider:a.provider, capability:a.capability, status:a.status, priority:a.priority, jobId:a.job_id, simulated:a.simulated, error:a.error, startedAt:a.started_at, completedAt:a.completed_at})),
    conversation: conversation ? {id:conversation.id, title:conversation.title, messages:messages.map((m) => ({id:m.id,role:m.role,content:m.content,createdAt:m.created_at}))} : undefined,
    deployments: deploymentHistory.map((d) => ({id:d.id, env:d.env, status:d.status, commit:d.commit_sha || "", url:d.url, at:d.created_at, adapter:d.adapter})),
    deploymentHistory: deploymentHistory.map((d) => ({id:d.id, env:d.env, status:d.status, commit:d.commit_sha || "", url:d.url, at:d.created_at, adapter:d.adapter})),
    brain: brainRow ? {vision:brainRow.vision, requirements:brainRow.requirements || [], decisions:brainRow.decisions || [], architecture:brainRow.architecture || [], schema:brainRow.schema || [], integrations:brainRow.integrations || [], tests:[], history:[]} : undefined,
    runs: runs.map((run) => ({id:run.id, prompt:run.prompt, provider:run.provider, model:run.model, status:run.status, startedAt:run.started_at, completedAt:run.completed_at, tokensIn:run.tokens_in || 0, tokensOut:run.tokens_out || 0, plan:steps.filter((s) => s.run_id === run.id), events:events.filter((e) => e.run_id === run.id).map((e) => ({id:e.id, at:e.created_at, level:e.level, stage:e.stage, message:e.message}))})),
    auditHistory: audit.map((e) => ({id:e.id, at:e.created_at, actor:e.actor === "user" ? "human" : e.actor, actorName:e.actor_name, action:e.action, target:e.target, risk:e.risk, approved:e.approved, diffSummary:e.diff_summary, stage:e.stage})),
    generatedFiles: files, lastGeneratedAt: latestSnapshotId ? latest[0]?.created_at : null
  }};
}
...
async function recordPlan(pool,runId) {
  ...
  const steps=[["Understand requirement","Read the requirement and current project context.","brain","low","done"],["Draft implementation plan","Translate the requirement into a bounded source change.","plan","low","done"],["Generate source","Produce a coherent runnable source tree.","build","medium","pending"],["Real build verification","Install dependencies under policy and run the allowed build command.","test","high","awaiting_approval"],["Review result","Record artifacts, tests, and audit evidence.","review","low","pending"]];
...
async function recordBuild(pool,{runId,projectSlug,prompt,artifacts,result,generation}) {
  ...
  if(!conversationId){conversationId=randomUUID();await pool.query("INSERT INTO conversations(id,project_id,title) VALUES($1,$2,$3)",[conversationId,projectId,"ForgeOS Project Conversation"]);}
  ...
  const brainVersion=Number(previousBrain?.version||0)+1; const brainVersionId=randomUUID();
  ...
  const providerAttemptId=randomUUID(); const executionProvider=result.provider || result.providerSelection?.selected || generation.provider || "http-executor";
  ...
  const snapshotId=randomUUID(); await pool.query("INSERT INTO source_snapshots(id,project_id,run_id,message) VALUES($1,$2,$3,$4)",[snapshotId,projectId,runId,"Generated source ("+generation.mode+") for run "+runId]);
  ...
  const passed=result.state==="passed"&&result.simulated===false; const testRunId=randomUUID();
  ...
  await pool.query("UPDATE conversations SET updated_at=now() WHERE id=$1",[conversationId]); return {projectId,snapshotId};
}

async function advanceRunSteps(pool,runId,{activeStage,status="running",completedStages=[]}={}) {if(!pool)return;if(completedStages.length) await pool.query("UPDATE ai_run_steps SET status='done' WHERE run_id=$1 AND stage=ANY($2::text[])",[runId,completedStages]);if(activeStage) await pool.query("UPDATE ai_run_steps SET status=$1 WHERE run_id=$2 AND stage=$3 AND status NOT IN ('done','rejected')",[status,runId,activeStage]);}
...
export async function buildAndPersist(pool,{runId,projectSlug,prompt,execute}) {
  ...
  const generation=await aiGenerate(prompt); const result=await execute(runId,generation.artifacts);
...
async function persistRepair(pool,{runId,prompt,files,failure,result,generation,projectSlug="forgeos"}) {
  if (!pool) return null; const project=(await pool.query("select id from projects where slug=$1 limit 1",[projectSlug])).rows[0]; if(!project)return null; const projectId=project.id; const snapshotId=randomUUID();
  ...
  await transitionRun(pool,runId,passed?"review":"repairing",{eventStage:"repair",message:passed?"Repair build verified; run is ready for review.":"Repair attempt failed; run remains in bounded repair."}); return {projectId,snapshotId,testRunId};
}
...
export async function autoRepairAndBuild({pool=null,runId,projectSlug="forgeos",prompt,files,failure,execute,maxAttempts=2,signal=null}) {
  let currentFiles=Array.isArray(files)?files:[]; let currentFailure=String(failure||"real build failed"); const attempts=Math.max(0,Math.min(2,Number(maxAttempts)||2)); const history=[];
  for(let attempt=1;attempt<=attempts;attempt++){
    if(signal?.aborted)return {state:"cancelled",simulated:false,repairAttempts:history};
    if(pool){const state=(await pool.query("SELECT status FROM ai_runs WHERE id=$1",[runId])).rows[0]?.status;if(state==="cancelled")return {state:"cancelled",simulated:false,repairAttempts:history};await pool.query("INSERT INTO ai_events(id,run_id,level,stage,message) VALUES($1,$2,'info','repair',$3)",[randomUUID(),runId,"Automatic repair attempt "+attempt+" of "+attempts+" started."]);}
    try{const result=await repairAndBuild({pool,runId,projectSlug,prompt,files:currentFiles,failure:currentFailure,execute,signal});history.push({attempt,state:result.state,provider:result.provider||null,error:result.state==="passed"?null:(result.error||"repair_failed")});if(result.state==="passed")return {...result,repairAttempts:history};currentFiles=result.sourceFiles?.length?result.sourceFiles:currentFiles;currentFailure=String(result.error||"repair build failed");}
    catch(error){currentFailure=error instanceof Error?error.message:String(error);history.push({attempt,state:"failed",provider:null,error:currentFailure});if(pool)await pool.query("INSERT INTO ai_events(id,run_id,level,stage,message) VALUES($1,$2,'warn','repair',$3)",[randomUUID(),runId,"Automatic repair attempt "+attempt+" failed: "+currentFailure.slice(0,1500)]);}
  }
  if(pool&&!signal?.aborted){await transitionRun(pool,runId,"failed",{eventStage:"repair",message:"Automatic repair attempts exhausted.",level:"error"}).catch(()=>{});await pool.query("INSERT INTO ai_events(id,run_id,level,stage,message) VALUES($1,$2,'error','repair',$3)",[randomUUID(),runId,"Automatic repair exhausted after "+attempts+" bounded attempt(s)."])}
...
export async function repairAndBuild({pool=null,runId,projectSlug="forgeos",prompt,files,failure,execute,signal=null}) {
  if(signal?.aborted)return {state:"cancelled",simulated:false}; const selected=await selectProvider(aiProviders,preferredAIProvider); let generated=null; let selectedProvider=null; const repairProjectId=pool?(await pool.query("SELECT project_id FROM ai_runs WHERE id=$1 LIMIT 1",[runId])).rows[0]?.project_id:null; const providerAttempts=[];
  for(const candidate of selected.candidates||[{provider:selected.provider,health:selected.health}]){if(signal?.aborted)return {state:"cancelled",simulated:false,providerAttempts};const provider=candidate.provider;if(typeof provider.repair!=="function"){providerAttempts.push({provider:provider.id,status:"skipped",error:"provider_cannot_repair"});continue;}const attemptId=randomUUID();if(pool)await pool.query("INSERT INTO provider_attempts(id,project_id,run_id,kind,provider,capability,status,priority,simulated) VALUES($1,$2,$3,'repair',$4,'ai','running',$5,false)",[attemptId,repairProjectId,runId,provider.id,candidate.health?.priority||0]).catch(()=>{});try{generated=await provider.repair({prompt,files,failure});if(signal?.aborted)return {state:"cancelled",simulated:false,providerAttempts};selectedProvider=provider;providerAttempts.push({provider:provider.id,status:"succeeded"});if(pool)await pool.query("UPDATE provider_attempts SET status='succeeded',completed_at=now(),observations=$1::jsonb WHERE id=$2",[JSON.stringify({model:generated.model,mode:generated.mode}),attemptId]).catch(()=>{});break;}catch(error){const message=error instanceof Error?error.message:String(error);providerAttempts.push({provider:provider.id,status:"failed",error:message});if(pool)await pool.query("UPDATE provider_attempts SET status='failed',completed_at=now(),error=$1 WHERE id=$2",[message,attemptId]).catch(()=>{});}}
  if(signal?.aborted)return {state:"cancelled",simulated:false,providerAttempts}; if(!generated||!selectedProvider)throw new Error("all_ai_repair_providers_failed:"+providerAttempts.map((a)=>a.provider+":"+a.error).join(";")); const artifacts=validateArtifacts(generated.artifacts); const result=await execute(runId,artifacts,{signal}); if(signal?.aborted)return {state:"cancelled",simulated:false,providerAttempts}; const persistence=await persistRepair(pool,{runId,prompt,files,failure,result,generation:{...generated,artifacts},projectSlug}); return {...result,repairSimulated:false,generationMode:"ai-repair",provider:generated.provider||selectedProvider.id,model:generated.model,sourceFiles:artifacts,projectId:persistence?.projectId||null,snapshotId:persistence?.snapshotId||null,testRunId:persistence?.testRunId||null,providerSelection:{selected:selectedProvider.id,preferred:preferredAIProvider,failover:selectedProvider.id!==preferredAIProvider},providerAttempts};
}

export async function latestSource(pool, projectSlug, runId=null) {
  if(!pool)return []; const snap=runId?await pool.query("SELECT ss.id FROM source_snapshots ss JOIN projects p ON p.id=ss.project_id WHERE p.slug=$1 AND ss.run_id=$2 ORDER BY ss.created_at DESC LIMIT 1",[projectSlug||"forgeos",runId]):await pool.query("SELECT ss.id FROM source_snapshots ss JOIN projects p ON p.id=ss.project_id WHERE p.slug=$1 ORDER BY ss.created_at DESC LIMIT 1",[projectSlug||"forgeos"]); if(!snap.rows[0])return []; const rows=await pool.query("SELECT path,content FROM source_files WHERE snapshot_id=$1 ORDER BY path",[snap.rows[0].id]); return rows.rows.map((r)=>({path:r.path,content:r.content}));
}
