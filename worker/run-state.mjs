...
export function canTransition(from, to) {
  return from === to || Boolean(ALLOWED[from]?.has(to));
}
export async function transitionRun(
  pool,
  runId,
  to,
  { eventStage = "state", message = null, level = "info", allowSame = true } = {},
) {
  ...
  try {
    ...
    const row = (
      await client.query("SELECT id,status FROM ai_runs WHERE id=$1 FOR UPDATE", [runId])
    ).rows[0];
    if (!row) throw new Error("run_not_found");
    if (row.status === to && allowSame) {
      await client.query("COMMIT");
      return row;
    }
    if (!canTransition(row.status, to))
      throw new Error("invalid_run_transition:" + row.status + "->" + to);
    const completed = TERMINAL.has(to);
    await client.query(
      "UPDATE ai_runs SET status=$2,completed_at=" +
        (completed ? "now()" : "NULL") +
        ",updated_at=now() WHERE id=$1",
      [runId, to],
    );
    await client.query(
      "INSERT INTO ai_events(id,run_id,level,stage,message) VALUES($1,$2,$3,$4,$5)",
      [randomUUID(), runId, level, eventStage, message || "Run state: " + row.status + " -> " + to],
    );
    await client.query("COMMIT");
