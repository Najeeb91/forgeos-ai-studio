...
export async function ensureProject(pool, projectSlug, prompt) {
  ...
  const result = await pool.query(
    ...
    [
      ...
      name,
      "AI software factory",
      "Project managed by ForgeOS. Initial requirement: " + String(prompt).slice(0, 1000),
      JSON.stringify(["React", "TypeScript", "Vite"]),
    ],
  );
...
const steps = [
  [
    ...
    "Read the requirement and current Project Memory/Brain.",
    "brain",
    "low",
    "done",
    false,
  ...
  [
    ...
    "Translate the requirement into a bounded implementation plan.",
    "plan",
    "low",
    "done",
    false,
  ...
  [
    "Generate source",
    "Generate or patch the source snapshot without destructive actions.",
    "build",
    "medium",
    "pending",
    false,
  ...
  [
    ...
    "Install dependencies under policy and execute the allowed build/test contract.",
    "test",
    "high",
    "awaiting_approval",
    true,
  ...
  [
    "Review result",
    "Record source, tests, provider observations and audit evidence.",
    "review",
    "low",
    "pending",
    false,
...
export async function prepareBuild(pool, { runId, projectSlug, prompt }) {
  ...
  if (!approval) {
    const gated = step.rows.find(
      (s) => s.status === "awaiting_approval" && ["high", "critical"].includes(s.risk),
    );
    if (gated) {
      const id = randomUUID();
      await pool.query(
      ...
      approval = (
        await pool.query(
          "SELECT id,step_id,action_type,target,reason,risk,status,created_at,decided_at FROM approval_requests WHERE id=$1",
          [id],
        )
      ).rows[0];
    }
  ...
  return {
    projectId,
    runId,
    state: approval ? "awaiting_approval" : "executing",
    plan: step.rows,
    approval,
  };
}
...
export async function approveBuild(pool, { runId, approvalId, decision, actorId = "user" }) {
  const approval = (
    await pool.query("SELECT * FROM approval_requests WHERE id=$1 AND run_id=$2 LIMIT 1", [
      approvalId,
      runId,
    ])
  ).rows[0];
  ...
  const normalized =
    decision === "approved" ? "approved" : decision === "rejected" ? "rejected" : null;
  ...
  await pool
    ...
    .catch(async () => {});
  await pool.query("INSERT INTO approvals(id,step_id,decision,actor) VALUES($1,$2,$3,$4)", [
    randomUUID(),
    approval.step_id,
    normalized,
    actorId,
  ]);
  await pool.query("UPDATE ai_run_steps SET status=$1 WHERE id=$2", [
    normalized === "approved" ? "done" : "rejected",
    approval.step_id,
  ]);
  const nextRunState =
    approval.action_type === "deploy_production"
      ? normalized === "approved"
        ? "review"
        : "failed"
      : normalized === "approved"
        ? "executing"
        : "failed";
  await transitionRun(pool, runId, nextRunState, {
    eventStage: "approval",
    message: "Human approval decision: " + normalized,
  });
  if (approval.action_type === "deploy_production" && normalized === "approved") {
    ...
    await pool.query(
      "UPDATE ai_run_steps SET status='running' WHERE run_id=$1 AND stage='test' AND status NOT IN ('done','rejected')",
      [runId],
    );
  }
  await pool.query(
