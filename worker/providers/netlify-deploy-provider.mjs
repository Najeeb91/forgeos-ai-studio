...
export class NetlifyDeployProvider extends ForgeOSProvider {
  constructor(secretsProvider = null) {
    super({ id: "netlify", capability: ProviderCapability.DEPLOY });
    this.secrets = secretsProvider;
  }
  async token() {
    const s = this.secrets ? await this.secrets.get("NETLIFY_AUTH_TOKEN") : null;
    return s?.value || process.env.NETLIFY_AUTH_TOKEN || "";
  }
  async health() {
    ...
    const r = await fetch(API + "/sites?per_page=1", {
    ...
    return {
      ...
      configured: true,
      realExecution: r.ok,
      error: r.ok ? null : "netlify_api_unavailable",
    };
  }
  async status({ token, deploymentId }) {
    ...
    const r = await fetch(API + "/deploys/" + encodeURIComponent(deploymentId), {
      headers: { Authorization: "Bearer " + token },
    });
    ...
    return {
      ...
      url: body?.ssl_url || body?.url || null,
      state: body?.state || "UNKNOWN",
      project: body?.site_name || null,
  ...
  async deploy({ token, projectName, files, environment = "production" }) {
    ...
    if (!siteId) {
      ...
      if (!created.ok && created.status !== 422)
        throw new Error(body?.message || body?.error || "netlify_site_create_failed");
      siteId = body?.id || "";
      if (!siteId) {
        const list = await fetch(API + "/sites?name=" + encodeURIComponent(name), {
          headers: { Authorization: "Bearer " + token },
        });
    ...
    for (const f of files) {
      if (!f?.path || f.path.startsWith("/") || f.path.includes("..") || f.path.includes("\\"))
        throw new Error("unsafe_netlify_path");
    ...
    const deployResponse = await fetch(
      API +
        ...
        "/deploys?" +
        new URLSearchParams({ production: environment === "production" ? "true" : "false" }),
      { method: "POST", headers, body: JSON.stringify({ files: manifest, async: false }) },
    );
    const deploy = await deployResponse.json().catch(() => ({}));
    if (!deployResponse.ok)
      throw new Error(deploy?.message || deploy?.error || "netlify_deploy_create_failed");
    const required = new Set(deploy.required || []);
    for (const sha of required) {
      const item = bySha.get(sha);
      if (!item) throw new Error("netlify_required_file_missing");
      const upload = await fetch(
        API +
          "/deploys/" +
          encodeURIComponent(deploy.id) +
          "/files/" +
      ...
      if (!upload.ok) {
        const b = await upload.json().catch(() => ({}));
        throw new Error(b?.message || "netlify_file_upload_failed");
      }
    }
    return {
      provider: this.id,
...
export function createNetlifyDeployProvider(secretsProvider = null) {
  return new NetlifyDeployProvider(secretsProvider);
}
