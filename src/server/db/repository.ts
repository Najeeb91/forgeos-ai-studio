...
function buildTree(rows: any[]): FileNode[] {
  ...
  for (const row of rows) {
    ...
    parts.forEach((part, i) => {
      ...
      if (existing) {
        if (existing.kind === "dir") level = existing.children ?? (existing.children = []);
        return;
      }
      ...
      const node: FileNode = isFile
        ? {
            ...
            status: row.status === "generated" ? "new" : (row.status ?? undefined),
...
export async function getRemoteProject(slug: string): Promise<Project | null> {
  ...
  const steps = runIds.length
    ? await sql`select * from ai_run_steps where run_id = any(${sql.array(runIds, "uuid")}) order by order_idx`
    : [];
  const events = runIds.length
    ? await sql`select * from ai_events where run_id = any(${sql.array(runIds, "uuid")}) order by created_at`
    : [];
  const stepIds = steps.map((s: any) => s.id);
  const approvals = stepIds.length
    ? await sql`select * from approvals where step_id = any(${sql.array(stepIds, "uuid")}) order by created_at`
    : [];
  const snapshot = snapshots[0];
  const files = snapshot
    ? await sql`select * from source_files where snapshot_id=${snapshot.id} order by path`
    : [];
  const testRun = snapshot
    ? (
        await sql`select * from test_runs where snapshot_id=${snapshot.id} order by created_at desc limit 1`
      )[0]
    : null;
  const testRows = testRun
    ? await sql`select * from test_results where test_run_id=${testRun.id} order by name`
    : [];
  ...
  const mappedRuns = runs.map((r: any) => ({
    ...
    status: (r.status === "testing" || r.status === "review"
      ? "completed"
      : r.status === "repairing"
        ? "failed"
        : r.status) as "running" | "awaiting_approval" | "completed" | "failed",
    tokensIn: r.tokens_in ?? 0,
    tokensOut: r.tokens_out ?? 0,
    plan: steps
      .filter((s: any) => s.run_id === r.id)
      .map((s: any) => ({
        id: s.id,
        title: s.title,
        detail: s.detail,
        ...
        risk: s.risk,
        status: s.status,
        decision: approvals.find((a: any) => a.step_id === s.id)?.decision,
      })),
    events: events
      .filter((e: any) => e.run_id === r.id)
      .map((e: any) => ({
        id: e.id,
        at: iso(e.created_at),
        level: e.level,
  ...
  const tests = testRows.map((t: any) => ({
    id: t.id,
    name: t.name,
    suite: t.suite === "execution" ? "integration" : t.suite,
    status: (t.status === "passed" ? "passing" : t.status === "failed" ? "failing" : "skipped") as
      "passing" | "failing" | "skipped" | "flaky",
  ...
  return {
    ...
    runs: mappedRuns,
    preview: {
      route: p.preview_route ?? "/preview",
      status: p.preview_status === "ready" ? "ready" : "cold",
      lastBuiltAt: p.preview_last_built_at ? iso(p.preview_last_built_at) : iso(p.updated_at),
    },
  } as Project;
}
export async function listRemoteProjects() {
  ...
  for (const row of rows) {
    const project = await getRemoteProject(row.slug);
    if (project) projects.push(project);
  }
  return projects;
}
export async function listRemoteProviders(): Promise<ProviderEntry[] | null> {
  return null;
}
