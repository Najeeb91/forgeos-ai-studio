...
function History() {
  ...
  const entries = useMemo(() => {
    ...
    const persistedAudit = (
      (project as typeof project & { auditHistory?: ChangeEntry[] }).auditHistory ?? []
    ).map((entry) => ({
      ...entry,
      source: "ai" as const,
    }));
    const aiEvents: Array<ChangeEntry & { source: "ai" }> = project.runs.flatMap((run) =>
      run.events.map((event) => ({
        ...
        target: event.stage,
        risk: (event.level === "error" || event.level === "warn"
          ? "high"
          : "low") as ChangeEntry["risk"],
        ...
        <div className="space-y-8">
          <section>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Durable checkpoints recovered from the ForgeOS project history. These are summaries of
              prior project work, not fabricated ChatGPT transcript messages.
            </p>
            <div className="mt-4 space-y-3">
              {contextHistory.length ? (
                ...
                <div className="py-4 text-sm text-muted-foreground">
                  No recovered project context yet.
                </div>
              )}
            </div>
          </section>
          <section>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Durable context merged from requirements, decisions, constraints, milestones, changes
              and outcomes.
            </p>
            <div className="mt-4 space-y-3">
              {(project.memory ?? []).length ? (
                (project.memory ?? []).map((entry) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      <Pill tone="neutral">{entry.kind}</Pill>
                      <Pill tone="info">{entry.authorType}</Pill>
                      <time className="font-mono text-[11px] text-muted-foreground">
                        {new Date(entry.occurredAt).toLocaleString()}
                      </time>
                    </div>
          ...
          <section>
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-2">
              ...
              <div className="rounded-lg border border-border bg-surface p-4">
                ...
                <div className="mt-3 space-y-2">
                  {(project.providerAttempts ?? []).length ? (
                    (project.providerAttempts ?? []).map((a) => (
                      <div key={a.id} className="rounded-md border border-border p-3">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-xs">{a.provider}</span>
                          <Pill
                            tone={
                              a.status === "succeeded"
                                ? "success"
                        ...
                        <div className="mt-1 text-[11px] text-muted-foreground">
                          {a.kind} · {a.capability} · simulated={String(a.simulated)}
                        </div>
                      </div>
                    ))
                  ) : (
          ...
          <section>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Every persisted Builder request appears here as the project’s working conversation.
            </p>
            <div className="mt-4 space-y-3">
              {conversation.length ? (
                conversation.map((run) => (
                  <article key={run.id} className="rounded-lg border border-border bg-surface p-4">
                    <div className="flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
                      <time>{new Date(run.at).toLocaleString()}</time>
                      <Pill tone="info">{run.status}</Pill>
                      <Pill tone="neutral">
                ...
                <div className="py-4 text-sm text-muted-foreground">
                  No persisted Builder conversation yet.
                </div>
              )}
            </div>
          </section>
            ...
            <div className="relative mt-4 ml-2 space-y-0 border-l border-border">
              {entries.length ? (
                ...
                <div className="flex items-center gap-2 py-6 text-sm text-muted-foreground">
                  ...
                  No persisted project activity yet.
                </div>
              )}
            </div>
          </section>
