  ...
  return (
    <PageBody>
      <Panel
        ...
        description="A controlled view of the generated application contract, not a live generated runtime."
        actions={<Pill tone="warning">seeded / simulated runtime</Pill>}
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            />{" "}
            Runtime {project.preview.status === "ready" ? "ready" : "cold"}
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button size="sm" variant="outline" onClick={() => setRefreshes((value) => value + 1)}>
              <RefreshCw />
              Refresh
            </Button>
          </div>
        ...
        <div className="mt-4 rounded-lg border border-border bg-background p-3">
          <div
            className={`${frameClass} overflow-hidden rounded-md border border-border bg-surface transition-all`}
          >
            <div className="flex items-center justify-between border-b border-border px-3 py-2 text-xs text-muted-foreground">
              <span>
                simulated://{project.slug}
                {project.preview.route}
              </span>
              <span>refresh {refreshes}</span>
            </div>
            <div className="grid min-h-[280px] place-items-center bg-grid-backdrop p-8 text-center">
              <div className="max-w-md">
                ...
                <p className="mt-4 text-xs text-warning">
                  Generated-app preview is simulated from seeded project data. No runtime has been
      ...
      <div className="grid gap-5 md:grid-cols-3">
        <Panel title="Runtime status">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <code>{project.preview.route}</code>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Source</span>
              <span>Project preview contract</span>
            </div>
            <div className="flex justify-between">
        ...
        <Panel title="Build status">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <Pill tone="warning">seeded metadata</Pill>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Last built</span>
              <span>{new Date(project.preview.lastBuiltAt).toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
        ...
        <Panel title="Release boundary">
          <div className="space-y-2 text-sm">
            <p className="text-muted-foreground">
              ForgeOS does not claim a successful build or runtime here.
            </p>
            {deployment ? (
              <a
              ...
              >
                Preview URL <ExternalLink className="size-3" />
              </a>
            ) : null}
          </div>
        </Panel>
