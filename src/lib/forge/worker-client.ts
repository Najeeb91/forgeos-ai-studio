...
export async function getLatestGeneratedSource(projectSlug: string): Promise<WorkerSourceFile[]> {
  ...
  try {
    payload = JSON.parse(text) as { files?: WorkerSourceFile[] };
  } catch {
    throw new Error(
      `Generated source lookup worker returned invalid JSON (HTTP ${response.status})`,
    );
  }
...
export async function getProjectFromWorker(projectSlug: string): Promise<{ project?: any } | null> {
  ...
  try {
    payload = JSON.parse(text) as { project?: any };
  } catch {
    throw new Error(`Project lookup worker returned invalid JSON (HTTP ${response.status})`);
  }
  return payload;
}
...
export async function approveBuild(
  ...
  try {
    ...
    throw new Error(`Approval worker returned invalid JSON (HTTP ${response.status})`);
  }
  if (!response.ok)
