...
export async function executeBuild(
  ...
  const payload = parseResponseJson(text) as WorkerBuildResponse | null;
  if (!response.ok)
    throw new Error(
      payload?.error ||
        payload?.build?.stderr ||
        payload?.install?.stderr ||
        `Execution worker failed with HTTP ${response.status}`,
    );
  if (!payload) throw new Error(`Execution worker returned invalid JSON (HTTP ${response.status})`);
...
export async function getLatestGeneratedSource(projectSlug: string): Promise<WorkerSourceFile[]> {
  ...
  const response = await fetch(`${url}/worker/source/latest`, {
    headers: { authorization: `Bearer ${token}`, "x-forgeos-project-slug": projectSlug },
  });
  const text = await response.text();
  const payload = parseResponseJson(text) as { files?: WorkerSourceFile[] } | null;
  if (!response.ok) throw new Error(`Generated source lookup failed (HTTP ${response.status})`);
  if (!payload)
    throw new Error(`Generated source lookup returned invalid JSON (HTTP ${response.status})`);
...
export async function getProjectFromWorker(projectSlug: string): Promise<{ project?: any } | null> {
  ...
  const text = await response.text();
  const payload = parseResponseJson(text) as { project?: any } | null;
  if (!response.ok) throw new Error(`Project lookup failed (HTTP ${response.status})`);
  if (!payload) throw new Error(`Project lookup returned invalid JSON (HTTP ${response.status})`);
  return payload;
}
...
export async function approveBuild(
  ...
  if (!response.ok)
    throw new Error(payload?.error || `Approval failed with HTTP ${response.status}`);
  if (!payload) throw new Error(`Approval worker returned invalid JSON (HTTP ${response.status})`);
...
export async function createForgeProject(input: {
  ...
  if (!payload)
    throw new Error("Project creation worker returned invalid JSON (HTTP " + response.status + ")");
...
export async function getDeploymentStatus(runId: string): Promise<any> {
  ...
  const payload = parseResponseJson(text);
  if (!response.ok)
    throw new Error(payload?.error || "Deployment status failed with HTTP " + response.status);
  if (!payload)
    throw new Error(
      "Deployment status worker returned invalid JSON (HTTP " + response.status + ")",
    );
...
export async function listProjectsFromWorker(): Promise<any[]> {
  ...
  const payload = parseResponseJson(text);
  if (!response.ok)
    throw new Error(payload?.error || "Project list failed with HTTP " + response.status);
  if (!payload)
    throw new Error("Project list worker returned invalid JSON (HTTP " + response.status + ")");
...
export async function recoverForgeRun(runId: string): Promise<any> {
  ...
  const payload = parseResponseJson(text);
  if (!response.ok)
    throw new Error(payload?.error || "Run recovery failed with HTTP " + response.status);
  if (!payload)
    throw new Error("Run recovery worker returned invalid JSON (HTTP " + response.status + ")");
...
export async function getForgeRun(runId: string): Promise<any> {
  ...
  const payload = parseResponseJson(text);
  if (!response.ok)
    throw new Error(payload?.error || "Run history lookup failed with HTTP " + response.status);
  if (!payload)
    throw new Error("Run history worker returned invalid JSON (HTTP " + response.status + ")");
