...
export type WorkerBuildResponse={ok?:boolean;jobId:string;runId:string|null;state:"passed"|"failed";phase:string;simulated:false;artifact?:{type:string;path:string}|null;install?:any;build?:any;policy?:any;error?:string};
...
export async function executeBuild(runId:string,files:WorkerSourceFile[]):Promise<WorkerBuildResponse>{
  ...
  const response=await fetch(`${url}/worker/jobs`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${token}`},body:JSON.stringify({runId,phase:"build",files,persistSource:true})});
