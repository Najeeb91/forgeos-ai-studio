...
import { getLatestGeneratedSource, recordBuildResult, recordBuildStart } from "./control-plane";
...
function generatedSource(prompt: string) {
  ...
  return [
    ...
    { path: "src/main.js", content: `import "./styles.css";
const request=${requestLiteral};const app=document.querySelector("#root");
app.innerHTML='<main class="shell"><section class="card"><div class="eyebrow">FORGEOS GENERATED SOFTWARE</div><h1>Working application scaffold</h1><p class="lead">A real source tree was generated, installed, and built by the authenticated ForgeOS execution worker.</p><div class="request"><strong>Requested change</strong><p>'+request+'</p></div><div class="status"><span></span> Build verified</div></section></main>';
` },
...
export const runForgeBuild=createServerFn({method:"POST"}).validator(buildInput).handler(async({data})=>{
  const files=generatedSource(data.prompt); const generationMode="deterministic-v2";
  await recordBuildStart(data.runId,data.prompt,generationMode);
  try {
    const result=await executeBuild(data.runId,files);
    await recordBuildResult(data.runId,files,result);
    return {...result,sourceFileCount:files.length,sourceFiles:files.map(f=>f.path),generationMode};
  } catch(error) {
    const message=error instanceof Error?error.message:"execution failed";
    await recordBuildResult(data.runId,files,{state:"failed",phase:"worker",build:{ok:false,stdout:"",stderr:message,error:message}});
    throw error;
  }
});
