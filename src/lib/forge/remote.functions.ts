...
import { createForgeProject, getProjectFromWorker, listProjectsFromWorker } from "./worker-client";
      ...
      if (remote?.project)
        return {
          project: {
            ...seed,
            ...remote.project,
            brain: remote.project.brain ?? seed.brain,
          } as typeof seed,
...
export const listForgeProjects = createServerFn({ method: "GET" }).handler(async () => {
  try {
    ...
    const mergedSeeds = seedProjects.map((seed) => ({
      ...seed,
      ...(bySlug.get(seed.slug) ?? {}),
      brain: bySlug.get(seed.slug)?.brain ?? seed.brain,
    }));
    const remoteOnly = remote.filter(
      (project) => !seedProjects.some((seed) => seed.slug === project.slug),
    );
